...
window.addEventListener('load', async () => {
    ...
    $('province-select').innerHTML = `<option selected hidden>Elegí tu provincia...</option>`;
    provincias.forEach(provincia => {
        $('province-select').innerHTML += `<option value="${provincia.nombre}" ${provincia.nombre === $('province').value && 'selected'}>${provincia.nombre}</option>`
    });
    $('province-select').addEventListener('change', async ({ target }) => {
        ...
        $('city-select').innerHTML = `<option selected hidden>Elegí tu localidad...</option>`;
